  ...
  try:
    ...
    for dirpath, _, files in os.walk(path):
      for filename in files:
        if filename in ("savedsearches.conf", "alert_actions.conf"):
    ...
    raise cex.ParsingError("The following error occurred while finding webhook URLs: {}".format(e))
...
def migrateLatencyTrackerLogIntervalSec(dryRun):
  ...
  def migrateHealthConf(healthConf, dryRun):
    ...
    confDict = comm.readConfFile(healthConf)
    if not stanzaName in confDict:
      return
    if not oldKeyName in confDict[stanzaName]:
      return
    confDict[stanzaName][newKeyName] = confDict[stanzaName][oldKeyName]
    del confDict[stanzaName][oldKeyName]
    if dryRun:
        logger.info("Would migrate file %s" % healthConf)
    else:
        logger.info("Migrating file %s" % healthConf)
        comm.writeConfFile(healthConf, confDict)
    return
...
def migrate_add_default_dashboard_xml_version_1_1(dryrun):
  ...
  for dashboard in dashboards:
    ...
    try:
      ...
      logger.error('{log} failed to parse the rest of the dashboard: `{e}`'.format(log=log, e=e))
      continue
    if dryrun:
      ...
      try:
        with open(dashboard, 'wb') as f:
          f.write(et.tostring(context.root, encoding='utf-8'))
      except Exception as e:
